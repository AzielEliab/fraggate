...
- `GET /v1/mesh` · `GET /v1/mesh/nodes` · `POST /v1/mesh/{enable,disable,join,heartbeat,leave,broadcast}` — suite mesh PROXY via `AZIEL_RUNTIME`. Default OFF. QNM-BUILD-1.0 live|locked|isolated. No Node Gate
...
| Live Nodes | `GET /v1/mesh` PROXY | Default OFF. QNM-BUILD-1.0 live\|locked\|isolated. No Node Gate. No auto-heal |
